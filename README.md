...
- **Create** a habit with a name, optional notes, a colour, and a weekly target
  of 1 to 7 days.
- **Read** all of your habits on one dashboard, newest first.
- **Update** any habit in place.
- **Delete** a habit, with an inline confirmation step. Its check-ins are removed
  with it.
- **Check in** on any of the last seven days by clicking that day's box. Clicking
  again undoes it.
- Each card shows the **current streak** and **progress against the weekly
  target**.
...
src/
  ...
  components/
    ...
    Dashboard.jsx       signed-in screen
    HabitForm.jsx       create and edit (same fields, one form)
    HabitCard.jsx       one habit: stats, check-ins, edit, delete
    CheckInRow.jsx      the seven day toggles
